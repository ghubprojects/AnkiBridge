...
public sealed class AnkiClient(HttpClient httpClient) : IAnkiClient
{
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            var request = new AnkiRequest(action, parameters ?? new { }, version);
            ...
            var response = await httpResponse.Content.ReadFromJsonAsync<AnkiResponse<T>>(cancellationToken);
