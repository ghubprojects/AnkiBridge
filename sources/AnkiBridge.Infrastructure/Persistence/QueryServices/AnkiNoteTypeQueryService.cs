...
public sealed class AnkiNoteTypeQueryService(
    ApplicationDbContext context)
    : IAnkiNoteTypeQueryService
{
    public async Task<PaginatedData<AnkiNoteTypeSearchResultDTO>> SearchAsync(
        string? keyword,
        int pageNumber,
        int pageSize,
        CancellationToken cancellationToken)
    {
        var query = context.AnkiNoteTypes.AsNoTracking();
        ...
        return await query
            ...
            .ToPaginatedDataAsync(
                pageNumber,
                pageSize,
                x => new AnkiNoteTypeSearchResultDTO(
                    x.Id,
                    x.Name),
                cancellationToken);
    }
